...
b. Bar chart displaying the counts of working class for all United States citizens.
...
c. Make a bivariate frequency table for the `workclass` variable as the rows and `race` as the columns.  In a second table, show the same table but with the marginal frequencies added.
...
d. Make a three-way frequency table for the `workclass`, `race`, and `sex` variable (have sex be the 3rd dimension). Then use `ftable()` to flatten the 3-D table.

```{r}
#xtabs( ~ workclass + race + sex, data = adult)
